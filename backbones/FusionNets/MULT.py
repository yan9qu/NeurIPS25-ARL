...
class MULT(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.text_subnet = BERTEncoder.from_pretrained(args.text_backbone)
    ...
    def forward(self, text_feats, video_feats, audio_feats):
        ...
        x_l = F.dropout(text.transpose(1, 2),
        ...
        last_hs_proj = self.proj2(F.dropout(F.relu(self.proj1(
            last_hs), inplace=True), p=self.output_dropout, training=self.training))
        last_hs_proj += last_hs

        logits = self.out_layer(last_hs_proj)

        return logits, last_hs, results
